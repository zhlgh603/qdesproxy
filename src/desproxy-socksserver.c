...
nothing_read (connection)
{
    ...
    if (recv (client_socket[connection], buffer, 1, 0) == -1)
    {
        perror ("read");
        EOC (connection);
        return;
...
method_accepted_v5 (connection)
{
    ...
    debug_printf (">method_accepted()\n");
    if (recv (client_socket[connection], buffer, 4, 0) == -1)
    {
        perror ("read");
        EOC (connection);
        return;
    ...
    if (buffer[3] == 1)
    {
        if (recv (client_socket[connection], buffer, 6, 0) == -1)
        {
            perror ("read");
            EOC (connection);
            return;
    ...
    if (buffer[3] == 3)
    {
        ...
        if (recv (client_socket[connection], buffer, 2, 0) == -1)
        {
            perror ("read");
            EOC (connection);
            return;
...
biconnected (connection)
{
    ...
    if ((count = recv (client_socket[connection], buffer, sizeof (buffer), 0)) == -1)
    {
    ...
    {
        if ((count = send (proxy_socket[connection], buffer, count, 0)) == -1)
        {
            perror ("write");
            EOC (connection);
            return;
            ...
            if (!client_socket_is_free[connection])
            {
                ...
                if (connection_status[connection] == BICONNECTED)
                {
                    if (FD_ISSET (proxy_socket[connection], &rmask))
                    {
                        ...
                        if ((count =
                             recv (proxy_socket[connection], buffer,
                                   sizeof (buffer), 0)) == -1)
                        {
                            perror ("read");
                            EOC (connection);
                        ...
                        else
                        {
                            ...
                            {
                                if ((count =
                                     send (client_socket[connection], buffer,
                                            count, 0)) == -1)
                                {
                                    perror ("write");
                                    EOC (connection);
